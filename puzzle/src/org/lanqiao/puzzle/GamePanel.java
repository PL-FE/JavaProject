...
public class GamePanel extends JPanel implements MouseListener{
    ...
    private static int col = 6;//设置这个拼图的行列
    private static Cell cells[] = new Cell[100];//创建一个按钮对象数组
    int ImageWidth;
    int ImageHeight;

    public GamePanel(int t,int i)//构造函数
    {	row = col = t;
        this.setLayout(null);
        init(i);
    }

    public void init(int ii)//初始化完成以下功能--完成图像的切割，完成图像到图标的转换，完成按钮图标的添加，将按钮添加到面板上，并且给每一个按钮添加监听机制
    {
        int num = 0;
        ...
        try
        {
            buf = ImageIO.read(new File("src/org/lanqiao/puzzle/img/"+ii+"_600px.jpg"));//读取文件图像
            ImageWidth = buf.getWidth();
            ImageHeight = buf.getHeight();
            width = ImageWidth/col;
            height = ImageHeight/row;
            JLabel PicMin = new JLabel("");
	        PicMin.setIcon(new ImageIcon(puzzle.class.getResource("/org/lanqiao/puzzle/img/"+ii+"_200px.jpg")));
	        PicMin.setBounds(651, 0, 200, 200);
	        this.add(PicMin);
        }catch(Exception e)
        {
    ...
    public void OutOfOrder()//乱序----打乱图片的排布顺序
    {
        ...
        {
            int index1 = random.nextInt(row*col);//cells的长度是9，但是他的上限是9，取不到9，所取值范围是0-8
            int index2 = random.nextInt(row*col);
    ...
    public boolean IsWin()//判断游戏玩家是否赢
    {
    	for(int i = 0; i < row; i++) {
			for(int j = 0; j < col; j++) {
				int num = i*col+j;
				int bntX = cells[num].getX();
				int bntY = cells[num].getY();
				int nowX = 600/col*j;
				int nowY = 600/row*i;
